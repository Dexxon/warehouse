...
public class Warehouse {
			...
			for(int i = 0;i < quantity;i++)
			{
				...
				for(int j = 0;j < locations.length;j++)
				{
					if(locations[j].getFreeSpace() > artikel.getVolume())
					{
						...
						artilcleToLocation.put(new Article(articleID), locations[j]);
						// System.out.println("Article eingelagert: " + artikel.getArtikelNummer() + " in " + j + " Freier Platz: " + locations[j].getFreierPlatz());
						break;
					}
				}
			}
	...
	{
		HashMap<Article,Location> articleToLocation = findArticlel(articleID, quantity);
		if(articleToLocation.size() >= quantity)
		{
			// System.out.println("Anzahl Article: " + zuordnungArtikelZuLagerplatz.size() + ". Zu löschen: " + anzahl);
		...
		// System.out.println("Anzahl Article nach Auslagerung: " + findeArtikel(artikelnummer).size());	
	}
